...
BEGIN
  ...
  INSERT INTO public.letters (
    ...
    primary_person, author, recipient, origin, destination, dateline, period, sheets,
    ...
    p_primary_person, p_author, p_recipient, p_origin, p_destination, p_dateline,
...
CREATE OR REPLACE FUNCTION public.search_letters(p_q text DEFAULT NULL::text, p_type text DEFAULT NULL::text, p_subtype text DEFAULT NULL::text, p_period text DEFAULT NULL::text, p_tstatus text DEFAULT NULL::text, p_review text DEFAULT NULL::text, p_scan text DEFAULT NULL::text, p_uncertain boolean DEFAULT false, p_id_status text DEFAULT NULL::text, p_date_precision text DEFAULT NULL::text, p_dig_status text DEFAULT NULL::text, p_tones text[] DEFAULT NULL::text[], p_view text DEFAULT NULL::text, p_research text DEFAULT NULL::text, p_person uuid DEFAULT NULL::uuid, p_org uuid DEFAULT NULL::uuid, p_event uuid DEFAULT NULL::uuid, p_date_from date DEFAULT NULL::date, p_date_to date DEFAULT NULL::date, p_author text DEFAULT NULL::text, p_recipient text DEFAULT NULL::text, p_place text DEFAULT NULL::text, p_sort text DEFAULT 'fh_seq'::text, p_dir text DEFAULT 'asc'::text, p_limit integer DEFAULT 100, p_offset integer DEFAULT 0, p_starred boolean DEFAULT false, p_salutation text DEFAULT NULL::text, p_addressee text DEFAULT NULL::text, p_closing text DEFAULT NULL::text, p_signature text DEFAULT NULL::text, p_postal text DEFAULT NULL::text, p_forwarded boolean DEFAULT false, p_health text DEFAULT NULL::text)
...
DECLARE
  ...
  v_sql text;
  v_ai_done text := '(EXISTS (SELECT 1 FROM public.ai_suggestions s WHERE s.letter_id = l.id) AND NOT EXISTS (SELECT 1 FROM public.ai_suggestions s WHERE s.letter_id = l.id AND s.status = ''pending''))';
BEGIN
  ...
  v_order := CASE p_sort
    ...
    WHEN 'dateline' THEN 'NULLIF(l.dateline, '''')'
  ...
  v_sql :=
    ...
    || '   AND ($4 IS NULL OR CASE WHEN $4 = ''needs'' THEN l.transcription_status NOT IN (''human_verified'',''not_required'') WHEN $4 LIKE ''!%'' THEN l.transcription_status <> substring($4 from 2) ELSE l.transcription_status = $4 END)'
    ...
    || '   AND ($21 IS NULL OR COALESCE(l.dateline, '''') || '' '' || COALESCE(l.origin, '''') || '' '' || COALESCE(l.destination, '''') || '' '' || COALESCE(l.forwarded_to, '''') ILIKE ''%'' || $21 || ''%'')'
    ...
    || '   AND ($30 IS NULL OR CASE WHEN $30 = ''paid'' THEN l.postal_service IN (''paid'',''airmail'') ELSE l.postal_service = $30 END)'
    || '   AND (NOT $31 OR COALESCE(l.forwarded, false))'
    || '   AND ($32 IS NULL OR CASE'
    || '         WHEN $32 = ''green'' THEN l.transcription_status = ''not_required'' OR (l.scan_status <> ''not_scanned'' AND l.transcription_status = ''human_verified'' AND ' || v_ai_done || ')'
    || '         WHEN $32 = ''purple'' THEN l.transcription_status = ''human_verified'' AND l.scan_status <> ''not_scanned'' AND NOT ' || v_ai_done
    || '         WHEN $32 = ''blue'' THEN l.transcription_status = ''ai_transcribed'' AND l.scan_status <> ''not_scanned'''
    || '         WHEN $32 = ''yellow'' THEN l.transcription_status NOT IN (''not_required'', ''human_verified'', ''ai_transcribed'', ''failed'') AND l.scan_status <> ''not_scanned'''
    || '         WHEN $32 = ''red'' THEN l.transcription_status <> ''not_required'' AND (l.scan_status = ''not_scanned'' OR l.transcription_status = ''failed'')'
    || '         WHEN $32 = ''needs_attention'' THEN NOT (l.transcription_status = ''not_required'' OR (l.scan_status <> ''not_scanned'' AND l.transcription_status = ''human_verified'' AND ' || v_ai_done || '))'
    || '         ELSE true END)'
    || '   AND ($22 IS NULL OR ('
    ...
    || '      OR COALESCE(l.dateline, '''') ILIKE ''%'' || $22 || ''%'''
    || '      OR COALESCE(l.origin, '''') ILIKE ''%'' || $22 || ''%'''
    || '      OR COALESCE(l.destination, '''') ILIKE ''%'' || $22 || ''%'''
    || '      OR COALESCE(l.forwarded_to, '''') ILIKE ''%'' || $22 || ''%'''
    || '      OR COALESCE(l.postal_service, '''') ILIKE ''%'' || $22 || ''%'''
    || '      OR COALESCE(l.postal_notes, '''') ILIKE ''%'' || $22 || ''%'''
  ...
  USING p_type, p_subtype, p_period, p_tstatus, p_review, p_scan,
        ...
        NULLIF(btrim(COALESCE(p_health, '')), '');
END $function$;