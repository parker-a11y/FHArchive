CREATE OR REPLACE FUNCTION public.search_letters(
  ...
  p_postal text DEFAULT NULL::text,
...
BEGIN
  ...
  v_sql :=
    ...
    || '         WHEN $32 = ''green'' THEN l.transcription_status = ''not_required'' OR (l.scan_status <> ''not_scanned'' AND l.transcription_status = ''human_verified'')'
    || '         WHEN $32 = ''yellow'' THEN l.transcription_status <> ''not_required'' AND l.scan_status <> ''not_scanned'' AND l.transcription_status NOT IN (''human_verified'', ''failed'')'
    || '         WHEN $32 = ''red'' THEN l.transcription_status <> ''not_required'' AND (l.scan_status = ''not_scanned'' OR l.transcription_status = ''failed'')'
    || '         WHEN $32 = ''needs_attention'' THEN l.transcription_status <> ''not_required'' AND (l.scan_status = ''not_scanned'' OR l.transcription_status <> ''human_verified'')'
    || '         ELSE true END)'
    || '   AND ($22 IS NULL OR ('
