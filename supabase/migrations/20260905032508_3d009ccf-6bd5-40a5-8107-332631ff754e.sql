...
BEGIN
  ...
  v_sql :=
    ...
    || '         WHEN $32 = ''green'' THEN l.transcription_status IN (''not_required'', ''human_verified'')'
    || '         WHEN $32 = ''yellow'' THEN l.scan_status <> ''not_scanned'' AND l.transcription_status NOT IN (''not_required'', ''human_verified'', ''failed'')'
    || '         WHEN $32 = ''red'' THEN l.scan_status = ''not_scanned'' OR l.transcription_status = ''failed'''
    || '         WHEN $32 = ''needs_attention'' THEN l.scan_status = ''not_scanned'' OR l.transcription_status NOT IN (''not_required'', ''human_verified'')'
    || '         ELSE true END)'
    || '   AND ($22 IS NULL OR ('
